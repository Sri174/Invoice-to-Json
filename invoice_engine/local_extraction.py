    ...
    for prep in preps:
        ...
        for psm in psm_opts:
            for oem in oem_opts:
                ...
                # build a small words list with positions for positional parsing
                words = []
                try:
                    for i, t in enumerate(data.get('text', [])):
                        ...
                        words.append({
                            'text': t,
                            'left': int(float(data.get('left', [0]*len(data.get('text', [])))[i])) if data.get('left') else 0,
                            'top': int(float(data.get('top', [0]*len(data.get('text', [])))[i])) if data.get('top') else 0,
                            'width': int(float(data.get('width', [0]*len(data.get('text', [])))[i])) if data.get('width') else 0,
                            'height': int(float(data.get('height', [0]*len(data.get('text', [])))[i])) if data.get('height') else 0,
                        })
                except Exception:
                    words = []
                confs = []
...
def local_extract_invoice(image_bytes: bytes, lang: Optional[str] = None, fast: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        # Use milder preprocessing: denoised grayscale (avoid aggressive adaptive thresholding
        # which can destroy fine text in some scans). Let `_ocr_with_variants` try stronger
        # preprocessing variants (including adaptive threshold) and positional parsing use
        # returned word boxes.
